...
1. Создайте генератор (как функцию или [класс-генератор](https://wiki.python.org/moin/Generators)), который:
   ...
   - Для каждого пикселя и каждого цветового канала (R,G,B) вычисляет распределение вероятностей значений на основе:
     * Данных о популярных стилях из файла styles.py
...
2. Генератор должен также возвращать вероятность генерации полученного стиля аватара

3. Выложите в репозиторий:
   - Код генератора
   - 5 примеров сгенерированных аватаров
   - Для каждого аватара укажите вероятность его генерации
